...
//                  | |                              
//  Version 1.1.3   |_| (c) Tele-Line Videotex Services
...
;(function($) {
	...
	$.scriptcam.getFrameAsBase64=function() {
		return eval(data.id).SC_getFrameAsBase64();
	}

	$.scriptcam.getMotionParameters=function() {
		eval(data.id).SC_getMotionParameters();
	}

	$.scriptcam.getBarCode=function() {
		return eval(data.id).SC_getBarCode();
	}
	
	$.scriptcam.startRecording=function() {
		eval(data.id).SC_startRecording();
	}
	
	$.scriptcam.closeCamera=function() {
		eval(data.id).SC_closeCamera();
	}
	
	$.scriptcam.changeVolume=function(value) {
		eval(data.id).SC_changeVolume(value);
	}

	$.scriptcam.sendMessage=function(value) {
		eval(data.id).SC_sendMessage(value);
	}

	$.scriptcam.changeCamera=function(value) {
		eval(data.id).SC_changeCamera(value);
	}

	$.scriptcam.changeMicrophone=function(value) {
		eval(data.id).SC_changeMicrophone(value);
	}
