...
$column="SELECT * ";
$sql=" FROM message_master  WHERE  (To_Id='".$_SESSION['SESS_ID']."' || From_Id='".$_SESSION['SESS_ID']."') AND Sent=1 AND ( From_Temp='".$_SESSION['SESS_ID']."' || To_Temp='".$_SESSION['SESS_ID']."' )	AND Temp_Del NOT LIKE '%".$_SESSION['SESS_ID']."%' 	";
...
if($_POST['btnSubmit']== 'Delete')
{
	...
	if(count($arr_id)>0)
	{
		...
		$sqlSS = "SELECT * FROM message_master WHERE Inbox_Id in ($str_id)";
		$resultSS = mysql_query($sqlSS);
		while($collesSS = mysql_fetch_array($resultSS))
		{
			if($collesSS['Temp_Del']!='')
			{
			$varTemp = $collesSS['Temp_Del'].','.$_SESSION['SESS_ID'];
			}
			else
			{
			$varTemp = $_SESSION['SESS_ID'];
			}
			$sql = "UPDATE message_master SET Temp_Del ='".$varTemp."' WHERE Inbox_Id in ($str_id)";
			mysql_query($sql);
		}
		$_SESSION['MSG'] = count($arr_id). ' messages deleted';
...
function doDeleteAction(form,acurl)
{
	...
	if (flag==10)
	{
		if(confirm("Are you sure you wants to permanent delete messages?"))
		{
			xx.action=acurl;
			xx.submit();
		}	
      ...
      <div class="content-box-2">
        ...
        <div class="rgtPannel2 fr">
          <div class="rgtPannel-col first">
            <img src="images/icon/trash.png" width="30" height="30"  align="absmiddle"  /><span style=" font-size:16px; font-weight:bold; padding-left:7px; top:5px; position:relative;">Trash</span>
            <div style="padding:10px 0 0 0;"><form action="" method="POST"  name="form1">
                  <table width="100%" border="0" cellspacing="2" cellpadding="2" >
					...
					{ 
					echo "<td colspan='3' height='250'  align='center'>No messages</td>";
					}
					?>
                    <?php if(mysql_num_rows($result)>0) { ?>
                    
                    <tr>
                      <td colspan="3" style="padding:10px 0 0 0;"><?php include "paging-in.inc.php";  ?></td>
                    </tr>
                    <tr>

                    <td colspan="3" style="padding:10px 0 0 0;">
                    <input name="btnClick" type="button" value="Delete" onClick="return doDeleteAction('form1','inbox.php');" style="background-color:#438EEB;  color:#fff;"/><input type="hidden" name="btnSubmit" value="Delete" >
