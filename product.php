...
$consumerKey = 'g7nGbcqQxBzRcR62b3N4GA';
$consumerSecret = 'mau0MVoLDZcLqFKX6aykwkhdazsWNcbBtGqBX7e5HGQ';
$oAuthToken = '520483705-2FUfMZXcKWfZVE8NhMdQBNkdSreoChLX013zQMw';
$oAuthSecret = 'QnzS9B6rphPHCwbSIam2T1IRSyJytA74CCuArFg';
...
{
$sql_notes = "SELECT sum(b.Membership_No+".$musician_primary_free_credit.") as notesTotal FROM membership_upgrade_master a JOIN membership_upgrade_history_master b ON (a.Membership_Upgrade_Id=b.Membership_Upgrade_Id) WHERE a.Status='1' AND b.Status='1' AND b.Member_Account_Id='".$_SESSION['SESS_ID']."'";
...
{
$TOTAL_NOTES = $musician_primary_free_credit-$collesMusicianCount['CMusicianTotal'];
}
...
////update record
function send_session_notification_emails($tmp_body)
{
	global $email_notification_settings;
	...
	$HEADER_from_admin = "From: ".addslashes(trim($_SESSION['SESS_FIRST_NAME']))." <".addslashes(trim($_SESSION['SESS_EMAIL']))."> \r\n";
	...
	$MAILSEND = @mail($TO, $SUBJECT, $BODY, $HEADER_from_admin.$HEADER);
	
	//========== Send an email to musician (owner of the session)
	$HEADER_from_other = "From: Mussino notification <notification@mussino.com> \n";
	$MAILSEND = @mail($_SESSION['SESS_EMAIL'], $SUBJECT, $BODY, $HEADER_from_other.$HEADER);
	
	//======-== Send an email to artist who have subscribed in the selected genre
	$get_subscribed_artists = mysql_query("SELECT mam.Email as email
	FROM `member_account_master` mam, `email_genre_settings` egs
	WHERE egs.user_id = mam.Member_Account_Id
	AND cat_id ='".$_POST['Category_Id']."'");
	while($subscribed_artists=mysql_fetch_array($get_subscribed_artists))
	{
		$MAILSEND = @mail($subscribed_artists['email'], $SUBJECT, $BODY, $HEADER_from_other.$HEADER);
	}
}
...
{ 
	...
	if($smallImage == '')
	{
		...
		if($imageStatus != '')
		{
			$user_img = "products/user_image/".$imageStatus;
			$expld = explode(".",$imageStatus);
			$new_nm = getuniqkey($imageStatus).".".$expld[count($expld)-1];
			$smallImage = $new_nm;
			copy($user_img,"products/product_image/".$new_nm);
		}
	}
	$name = $_REQUEST['name'];
			  ...
			  if($arrayDate['days']<=7)	
			  {
				 ...
				 $query = "INSERT INTO product_master SET
					 ...
					 executeQuery($query);
					  ...
					  $message = 'Session Title : '.trim($_POST['Title']).' - ';
					  $message .= trim($_POST['Product_Notes']).' credits songwriters need to enter this session. ';
					  $message .= 'Session Date : '.$Start_Date.' To '.$End_Date;
				...
				$BODY = "
					<table width='100%' border='0' cellspacing='0' cellpadding='0' style='background:#E0E0E0; border:#999999 1px solid;'>
						<tr><td>Title : ".addslashes(trim($_REQUEST['Title']))."</td></tr>
						<tr><td>Product Notes : ".addslashes(trim($_REQUEST['Product_Notes']))."</td></tr>
						<tr><td>Sound : ".Get_Single_Field("sound_type_master","Sound_Type_Name","Sound_Type_Id","$_POST[Sound]")."</td></tr>
						<tr><td>Genre : <a href='".SITE_WS_MPATH."/session.php?id=".$_POST[Category_Id]."'>".Get_Single_Field("category_master","Category_Name","Category_Id","$_POST[Category_Id]")."</a></td></tr>
						<tr><td>Details : ".trim($_REQUEST['Short_Desc'])."</td></tr>
						<tr><td>Session Start : ".$Start_Date."</td></tr>
						<tr><td>Session End : ".$End_Date."</td></tr>
					</table>
				";
				send_session_notification_emails($BODY);
	...
	elseif($TOTAL_NOTES+5>5)
	{
				if($arrayDate['days']<=7)	
				{
					...
					/*			
					$BODY  = "Title : ".addslashes(trim($_REQUEST['Title']))." <br>";
					$BODY .= "Product Notes : ".addslashes(trim($_REQUEST['Product_Notes']))." <br>";
					$BODY .= "Sound : ".Get_Single_Field("sound_type_master","Sound_Type_Name","Sound_Type_Id","$_POST[Sound]")." <br>";
					$BODY .= "Genre : <a href='".SITE_WS_MPATH."/session.php?id=".$_POST[Category_Id]."'>".Get_Single_Field("category_master","Category_Name","Category_Id","$_POST[Category_Id]")."</a> <br>";
					$BODY .= "Details : ".trim($_REQUEST['Short_Desc'])." <br>";
					$BODY .= "Session Start : ".$Start_Date." <br>";
					$BODY .= "Session End : ".$End_Date." <br>";
					*/
					$BODY = "
					<table width='100%' border='0' cellspacing='0' cellpadding='0' style='background:#E0E0E0; border:#999999 1px solid;'>
						<tr><td>Title : ".addslashes(trim($_REQUEST['Title']))."</td></tr>
	...
	$(function(){
		...
		new AjaxUpload(btnUpload, {
			...
			onSubmit: function(file, ext){
				 ...
				 if ( !(ext && /^(flv|FLV|mp3|MP3)$/.test(ext))){ 
                    // extension is not allowed 
...
<div class="content-box-2">
       ...
       <!-- CREDIT BOX -->    
             ...
             <div class="form-container">
              ...
              <ul>
                ...
                <li>
                  ...
                  <input type="text" name="Title" id="Title" placeholder="Session Name" value="<?=$Title?>" class="" size="30" />
              ...
              <?php /*?> <?php echo $collesMemberShip['Membership_Upgrade_Id'] ?><?php */?>
                <li>
                  <div style="margin-top:25px"><p style="font-size:12px;">Free account only have 1 option. Upgrade to <a href="http://www.mussino.com/membership-upgrade.php" target="_blank">Business account</a> for more options</p></div> 
                  <div class="input-2">
                ...
                <?php /*?><li class="formBG2">
                  <div class="caption-2" style="width:300px;">Description</div>
          ...
          <div class="time-wrapper" style="display:none;">     
           <li>
                ...
                <h1><img src="images/tick.png" width="50" align="absmiddle" />Already set for a 7 day session</h1>
                  <div id="hideme" class="sessionTime" style="width:500px"><span style="padding-right:5px;"><img src="images/date.png" width="40" height="40" align="absmiddle" /></span><strong>Start Date </strong> <?php echo date('l F j, Y'); ?></div>
                  <div class="input-2">
              ...
              <li id="hideme1" class="sessionTime formBG0">
                  <div style="width:500px"><span style="padding-right:5px;"><img src="images/date1.png" width="40" height="40" align="absmiddle" /></span><strong>End Date</strong> <?php echo date('l F j, Y', strtotime("+7 days")); ?> </div>
                  <div class="input-2">
                  <table cellpadding="0" cellspacing="0" width="380" border="0">
                   <tr>
                     <td width="68" align="left" style="color:#25A6D5; font-weight:bold;">Day</td>
                  ...
                  <!-- <tr>
                   ...
                   <input type="hidden" name="ed"  id="ed" value="<?=$eDay?>" />
                    <input type="hidden" name="em"  id="em" value="<?=$eMon?>" />
                    <input type="hidden" name="ey"  id="ey" value="<?=$eYear?>" />
                    <input type="hidden" name="ehh" id="ehh" value="<?=$eHour?>" />
                    <input type="hidden" name="emm" id="emm" value="<?=$eMinute?>" />
                    <input type="hidden" name="ess" id="ess" value="<?=$eSecond?>" />
                   </table> 
                ...
                <li class="" style="width:600px;">
                  ...
                  <span style="font-family:Arial, Helvetica, sans-serif; font-size:18px; color: #069"><br />
                ...
                <?php if($_REQUEST['id']=='') { ?>
                 <div style="font-size:16px; color:#0099CC; text-transform:uppercase; margin-top:15px;">electronic signature</div>
    ...
    font-weight: bold;height: 25px;margin-top: 2px;padding: 0 5px; width: 100px;">
                ...
                <li>
                 <?php /*?> <div style="font-family:Arial, Helvetica, sans-serif; font-size:13px; color: #F00; padding: 10px 0 10px 0;"><strong>Note : Only admin can delete an active session. <br> Send email to editsession@mussino.com</strong></div><?php */?>
                  <div class="cl"></div>
                </li>
                <li>
                  <div class="input-1">
