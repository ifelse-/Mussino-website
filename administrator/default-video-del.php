...
if(count($_POST['ids'])>0)
{
	...
	$sql1 = "SELECT * FROM default_video_master WHERE Video_Id  in (".$ids.") ";
	...
	unlink($target_path);
	 ...
	 $sql = "DELETE FROM default_video_master WHERE Video_Id in (".$ids.") ";  
	 executeQuery($sql);
	...
	{
	  $sql = "UPDATE default_video_master  SET Status='1' WHERE Video_Id in (".$ids.")";
	  executeQuery($sql);
   ...
   {
    $sql = "UPDATE default_video_master SET Status='0' WHERE Video_Id in (".$ids.")";
    executeQuery($sql);
