...
if($_GET['order_by']=='') { $order_by="Category_Name"; } else { $order_by=$_REQUEST['order_by'];}
if($_GET['order_by2']=='') { $order_by2="DESC"; } else { $order_by2=$_REQUEST['order_by2'];}


$column="SELECT * ";
$sql=" FROM category_master WHERE 1=1  ";
...
{
$sql.=" AND Parent_Id='".$_GET['pid']."' ";
}
	
if($_REQUEST['alphabet']!='')
{
$sql.= " and Category_Name like '". $_GET['alphabet'] ."%'";
}
      ...
      <div class="detail-col_inner">
        <div class="box-1">
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table-1">
            ...
            <tr>
              <td width="150" align="left" valign="middle">Category :</td>
              <td align="left" valign="top" class="input-1">
              <input type="text" name="Category_Name" size="40"  value="<?=stripslashes($_REQUEST['Category_Name'])?>" class="textbox"/>              </td>
            </tr>
        ...
        <tr align="right">
		 ...
		 <a href="category.php" class="lin2">Add New Category</a>
          </td>
           <? if($reccnt<=0) { ?>
		  </tr>
          ...
          <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table-1">
            <tr>
              ...
              <th width="250" align="center" valign="top">Category Name  <?=sort_arrows('Category_Name')?></th>
			...
			{
		     $sql_count = "SELECT count(Category_Id) as total FROM category_master WHERE Parent_Id ='".$row['Category_Id']."'";
			 $result_count = mysql_query($sql_count);
			 $colles_count = mysql_fetch_array($result_count);
			 $countSub = $colles_count['total'];
			?>
            <tr>
              <td width="50" align="center" valign="top"><input name="ids[]" type="checkbox" value="<?=$row['Category_Id']?>" />  </td>
              <td width="250" align="center" valign="top" ><?=stripslashes(trim($row['Category_Name']));?></td>
              ...
              <?php if($_GET['pid']=='') { ?>
              <a href="category.php?id=<?=$row['Category_Id']?>"><img src="images/editrec.gif" width="12" height="12" border="0"></a>  
              <?php } else { ?>
              <a href="category.php?id=<?=$row['Category_Id']?>&pid=<?=$_GET['pid']?>"><img src="images/editrec.gif" width="12" height="12" border="0"></a>
              <?php } ?>
              </td>
