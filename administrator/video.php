...
$list_page ='video-list.php';
$head_page ='video.php';

$sql = "SELECT * FROM video_master WHERE Video_Id ='".$_GET['id']."'";
...
{ 
  ...
  if($_REQUEST['id']!='')
	{
		$sql = "SELECT * FROM video_master WHERE Video_Name = '".$Video_Name."' AND Display = '".$Display."'  AND Video_Id !='".$_REQUEST['id']."'";
		...
		{	
			...
			if(!empty($_FILES['Video_File']['name']))
			{
				...
				$lastId = $_GET['id'];
				$create_name = "Video_File_".$lastId;
				$new_filename = $create_name.".".$getext;
				$upload_path = "../products/video_file/".$new_filename;
		  ...
		  /* if(!empty($_FILES['Video_Image']['name']))
			{
				...
				$upload_path = "../products/video_file/".$new_filename_image;
		   ...
		   {
		   $new_filename_image = $colles['Video_Image'];
		   }*/
			
			$sql = "UPDATE video_master SET 
	...
	else
	{
			...
			$sql = "SELECT * FROM video_master WHERE Video_Name = '".$Video_Name."' AND Display = '".$Display."' ";
			$result = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
			if(mysql_num_rows($result)>0)
			{
				$_SESSION['sess_mess'] = "Video already exist";
			...
			else
			{
				...
				$sql = "INSERT INTO video_master SET
						Video_Name = '$Video_Name',
						Video_Embded_File = '".addslashes($Video_Embded_File)."',
					    Display = '".$Display."',
						Video_Date = now(),
				...
				if(!empty($_FILES['Video_File']['name']))
				{
					...
					$upload_path = "../products/video_file/".$new_filename;
					move_uploaded_file($_FILES['Video_File']['tmp_name'],$upload_path);
					$sql_update = mysql_query("UPDATE video_master SET Video_File='".$new_filename."' WHERE Video_Id='".$lastId."'");
	     	    }
				/*if(!empty($_FILES['Video_Image']['name']))
				{
					...
					$upload_path = "../products/video_file/".$new_filename_image;
					move_uploaded_file($_FILES['Video_Image']['tmp_name'],$upload_path);
					$sql_update = mysql_query("UPDATE video_master SET Video_Image='".$new_filename_image."' WHERE Video_Id='".$lastId."'");
	     	    }*/
...
<div class="content-container">
    ...
    <div class="detail-col fr">
      <div class="detail-col_inner">
       ...
       <div class="box-1">
       <h2> Index Top Audio Or Video <?=$_GET['id']!='' ? 'Edit' : 'Add';?></h2>
        ...
        <? } ?>
         ...
         <?php if($_GET['id']!='') { ?>
          <tr>
			...
			<?php 
			if(file_exists("../products/video_file/$dataColles[Video_File]") && $dataColles['Video_File']!='') 
			{ 
			echo $dataColles['Video_File'];
			?> 
            &nbsp;&nbsp;&nbsp;&nbsp;<a href="video-del-file.php?id=<?=$dataColles['Video_Id']?>" title="Delete File"><img src="images/cross.gif" width="16" height="16" align="absmiddle" /></a>
            <?php
            } 
