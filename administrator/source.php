...
$list_page ='source-list.php';
$head_page ='source.php';
...
{ 
  ...
  if($_REQUEST['id']!='')
	{
		$sql = "SELECT * FROM source_link_master WHERE Source_Name = '$Source_Name'  AND Source_Id !='".$_REQUEST['id']."'";
		$result = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
		if(mysql_num_rows($result)>0)
		{
			$_SESSION['sess_mess'] = "Source already exist";
			header("location: ". $head_page."?id=".$_REQUEST['id']);
		    exit;
		}
		else
		{	
			$sql = "UPDATE source_link_master SET 
					Source_Name = '$Source_Name',
					Source_Link = '$Source_Link',
					Status = '$Status'
					WHERE Source_Id = '".$_REQUEST['id']."'";
			$result = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
			$_SESSION['sess_mess'] = "Source updated successfully.";
	...
	else
	{
			...
			$sql = "SELECT * FROM source_link_master WHERE Source_Name = '$Source_Name'  ";
			$result = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
			if(mysql_num_rows($result)>0)
			{
				$_SESSION['sess_mess'] = "Source already exist";
			...
			else
			{
				...
				$sql = "INSERT INTO source_link_master SET
						Source_Name = '$Source_Name',
						Source_Link = '$Source_Link',
						Status = '$Status'";
				$result = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
				$_SESSION['sess_mess'] =  "Source successfully added";
...
if($_GET['id']!='')
{
$sql = "SELECT * FROM source_link_master WHERE Source_Id ='".$_GET['id']."'";
$result = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
...
{
	if (placeadd.Source_Name.value == "") 
	{
		alert("\nPlease enter source name.")
		placeadd.Source_Name.focus();
		return false;
	}
	else if (placeadd.Source_Link.value == "") 
	{
		alert("\nPlease enter source link.")
		placeadd.Source_Link.focus();
		return false;
	}
  ...
  <div class="content-container">
    ...
    <div class="detail-col fr">
      <div class="detail-col_inner">
       ...
       <div class="box-1">
       <h2> Source Link <?=$_GET['id']!='' ? 'Edit' : 'Add';?></h2>
       <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table-1">
        ...
        <? } ?>
          <tr>
            <td width="200" align="left" valign="middle">Source Name</td>
            <td align="left" valign="top" class="input-1"><input type="text" name="Source_Name" value="<?=stripslashes(trim($dataColles['Source_Name']));?>"  size="50" class="textbox"></td>
          </tr>
          
          <tr>
            <td width="200" align="left" valign="middle">Source Link</td>
            <td align="left" valign="top" class="input-1"><input type="text" name="Source_Link" value="<?=trim($dataColles['Source_Link']);?>"  size="50" class="textbox"></td>
          </tr>
          ...
          <tr>
            <td align="left" valign="middle"><input type="hidden" name="id" value="<?=$dataColles['Source_Id'];?>" /></td>
