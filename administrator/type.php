...
{ 
  ...
  @extract($_POST);
  @extract($_REQUEST);
  $Type_Name = addslashes($Type_Name);
    
  if($_REQUEST['id']!='')
	{
		$sql = "SELECT * FROM type_master WHERE Type_Name = '$Type_Name'  AND Type_Id !='".$_REQUEST['id']."'";
		$result = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
		if(mysql_num_rows($result)>0)
		{
			$_SESSION['sess_mess'] = "Type already exist";
			header("location: ". $head_page."?id=".$_REQUEST['id']);
		    exit;
		}
		else
		{	
			$sql = "UPDATE type_master SET 
					Type_Name = '$Type_Name',
					OptionType = '$OptionType',
					Status = '$Status'
					WHERE Type_Id = '".$_REQUEST['id']."'";
			$result = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
			$_SESSION['sess_mess'] = "Type updated successfully.";
	...
	else
	{
			...
			$sql = "SELECT * FROM type_master WHERE Type_Name = '$Type_Name'  ";
			$result = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
			if(mysql_num_rows($result)>0)
			{
				$_SESSION['sess_mess'] = "Type already exist";
			...
			else
			{
				...
				$sql = "INSERT INTO type_master SET
						Type_Name = '$Type_Name',
						OptionType = '$OptionType',
						Status = '$Status'";
				$result = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
				$_SESSION['sess_mess'] =  "Type successfully added";
...
if($_GET['id']!='')
{
$sql = "SELECT * FROM type_master WHERE Type_Id ='".$_GET['id']."'";
$result = mysql_query($sql) or die('<br>'.$sql.'<br>'.mysql_error());
...
{
	if (placeadd.Type_Name.value == "") 
	{
		alert("\nPlease enter type name.")
		placeadd.Type_Name.focus();
		return false;
	}
    ...
    <div class="detail-col fr">
      <div class="detail-col_inner">
       ...
       <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table-1">
        ...
        <? } ?>
          <tr>
            <td width="200" align="left" valign="middle">Type Name</td>
            <td align="left" valign="top" class="input-1"><input type="text" name="Type_Name" value="<?=stripslashes(trim($dataColles['Type_Name']));?>"  size="50" class="textbox"></td>
          </tr>
          
          <tr>
            <td align="left" valign="middle">Allow for Member</td>
            <td align="left" valign="top" class="input-1">
              <select name="OptionType" size="1" class="textbox">
                  <option value="A" <? if($dataColles['OptionType']=='A') { echo "SELECTED";}?>>Artist</option>
                  <option value="M" <? if($dataColles['OptionType']=='M') { echo "SELECTED";}?>>Musician</option>
                </select>
            </td>
          </tr>
          ...
          <tr>
            <td align="left" valign="middle"><input type="hidden" name="id" value="<?=$dataColles['Type_Id'];?>" /></td>
