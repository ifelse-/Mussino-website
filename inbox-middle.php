...
$column="SELECT * ";
$sql=" FROM message_master  WHERE  To_Id='".$_SESSION['SESS_ID']."' AND Sent=1 AND To_Temp!='".$_SESSION['SESS_ID']."'	";
...
if($_POST['btnSubmit']== 'Delete')
{
	...
	if(count($arr_id)>0)
	{
		...
		$sql = "UPDATE message_master SET To_Temp='".$_SESSION['SESS_ID']."' WHERE Inbox_Id in ($str_id)";
		mysql_query($sql);
            ...
            <div style="padding:10px 0 0 0;"><form action="" method="POST"  name="form1">
                  <table width="100%" border="0" cellspacing="2" cellpadding="2" >
                    ...
                    <tr bgcolor="<?=$bgcolor?>">
                      ...
                      <td class="title-2" title="Show Message"><a href="inbox.php?id=<?=$line['Inbox_Id']?>"><?=$line['Read_Status']==0?'<strong>'.ucwords(strtolower(stripslashes(Get_Single_Field("member_account_master","First_Name","Member_Account_Id","$line[From_Id]").' '.Get_Single_Field("member_account_master","Last_Name","Member_Account_Id","$line[From_Id]")))).'</strong>':ucwords(strtolower(stripslashes(Get_Single_Field("member_account_master","First_Name","Member_Account_Id","$line[From_Id]").' '.Get_Single_Field("member_account_master","Last_Name","Member_Account_Id","$line[From_Id]"))));?></a></td>
                      <td class="title-2" title="Show Message"><a href="inbox.php?id=<?=$line['Inbox_Id']?>"><?=$line['Read_Status']==0?'<strong>'.stripslashes($line['Subject']).'</strong>':stripslashes($line['Subject']);?></a></td>
                      <td class="date"><div><?=$line['Read_Status']==0?'<strong>'.date('m-d-Y',strtotime($line['Date'])).'</strong>':date('m-d-Y',strtotime($line['Date']))?></div></td>
                    </tr>
                    <?php
					$k++;
					}
