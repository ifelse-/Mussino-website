...
if(isset($_REQUEST['js'])){
	...
	if(typeof(jQuery)=='undefined'){
		document.write('Send to Friend: Please include jQuery script on your page <strong>before</strong> send to friend: &lt;'+'scri'+'pt type="text/javascript" src="<?php echo $script_dir;?>jquery-latest.js"&gt;&lt;'+'/sc'+'ript&gt;');
	}
	...
	var GB_ANIMATION = true;
      $(document).ready(function(){
        $("a.sendtofriend").click(function(){
          ...
          h = h.replace(/\#.*/,'');
          GB_show(t,this.href+(this.href.match('url=') ? '&' : '?url='+h+'&')+'lb',<?php echo $popup_height;?>,<?php echo $popup_width;?>,'<?php echo $script_dir;?>');
          return false;
        });
      });
	<?php
...
if($restrict_domain){
	if($restrict_domain===true)$restrict_domain = $_SERVER['HTTP_HOST'];
...
if(isset($_REQUEST['send'])){
    ...
    if($do_recaptcha){
        ...
        if (!$resp->is_valid) {
            $errors[] = "Please enter correct reCAPTCHA code (two words)";
            $do_send = false;
        }
    }
	...
	foreach($friends as $key=>$val){
		...
		if(!$val || !strpos($val,'@')){
			unset($friends[$key]);
            $errors[] = htmlspecialchars($original_val)." is not a valid friends email address.";
            $do_send = false;
		}
	}
    $your_email = trim($_REQUEST['your_email']);
    if(function_exists('filter_var')){
        $your_email = filter_var($your_email, FILTER_VALIDATE_EMAIL);
    }

	if($do_send && $url && count($friends) && strpos($your_email,'@')){ // bad validation, meh
		...
		foreach($friends as $friend){
			...
			$replace = array(
				'FROM' => ($_REQUEST['your_name'] ? $_REQUEST['your_name'] . ' (' . $your_email . ')' : $your_email),
