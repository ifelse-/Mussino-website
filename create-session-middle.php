...
if($_POST['flvnm'] != '')
{
	//========= User is trying to upload recorded stream=============
	$video_time = 0;
	$cmd = 'ffmpeg -i '.$wmspath.$_POST['flvnm'].'.flv 2>&1';
	//$cmd = 'C:/xampp5/htdocs/marvin/older/old/ffmpeg/ffmpeg.exe -i "'.$wmspath.$_POST['flvnm'].'.flv" 2>&1';
	...
	$video_time = ceil($duration_array[0] * 3600 + $duration_array[1] * 60 + $duration_array[2]);

	//ffmpeg -i omegaitech_996.flv -f image2 -vframes 1 /var/www/vhosts/ip-50-63-189-96.ip.secureserver.net/dubover.com/dev/session_images/omegaitech_996.png
	
	exec("ffmpeg -i ".$wmspath.$_POST['flvnm'].".flv -f image2 -vframes 1 ".$session_img_path.$_POST['flvnm'].".png");
	//exec('C:/xampp5/htdocs/marvin/older/old/ffmpeg/ffmpeg.exe -i "'.$wmspath.$_POST['flvnm'].'.flv" -f image2 -vframes 1 '.$session_img_path.$_POST['flvnm'].".png");
	
	mysql_query("insert into new_recording_cam set record_date=now(),ses_id='".$_POST['ses_id']."',video_time='$video_time',descr='".addslashes($_POST['descr'])."',song_title='".addslashes($_POST['title'])."',record_user_id='".$_SESSION['SESS_ID']."',flv_name='".$_POST['flvnm']."'");
	$sql_bank ="INSERT INTO my_bank SET
                    Product_Id ='".trim($_POST['ses_id'])."',
					Notes_Value ='1',
					To_Member_Account_Id ='0',
					From_Member_Account_Id ='".$_SESSION['SESS_ID']."',
				    Account_Type ='Artist'";
        mysql_query($sql_bank);
	if($email_notification_settings == 'yes')
	{
		//========= Send email notification to the session owner
		$get_session_owner_info = mysql_query("
		SELECT mam.Email,mam.First_Name,mam.Last_Name,pm.Title,pm.Category_Id FROM `product_master` pm,member_account_master mam WHERE mam.Member_Account_Id=pm.Member_Account_Id and pm.Product_Id='".$_POST['ses_id']."'
		");
		$sofname = "";
		$solname = "";
		$soemail = "";
		$sestitle = "";
		$cat_id = "";
		while($session_owner_info = mysql_fetch_array($get_session_owner_info))
		{
			$sofname = $session_owner_info['First_Name'];
			$solname = $session_owner_info['Last_Name'];
			$soemail = $session_owner_info['Email'];
			$sestitle = $session_owner_info['Title'];
			$cat_id = $session_owner_info['Category_Id'];
		}
		if($soemail != '')
		{
			$body = "
			<table width='100%' border='0' cellspacing='0' cellpadding='0' style='background:#F2F2F2; border:#CCCCCC;'>
				<tr>
					<td>Hello ".$sofname." ".$solname."</td>
				</tr>
				<tr><td height='6'></td></tr>
				<tr>
					<td>".$_SESSION['SESS_FIRST_NAME']." ".$_SESSION['SESS_LAST_NAME']." have recorded new video in your below session.</td>
				</tr>
				<tr><td height='6'></td></tr>
				<tr>
					<td><strong>Session:</strong> <a href='".SITE_WS_MPATH."/session.php?id=".$cat_id."'>".$sestitle."</a></td>
				</tr>
				<tr><td height='6'></td></tr>
				<tr>
					<td><strong>Genre:</strong> <a href='".SITE_WS_MPATH."/session.php?id=".$cat_id."'>".Get_Single_Field("category_master","Category_Name","Category_Id","$cat_id")."</a></td>
				</tr>
				<tr><td height='6'></td></tr>
				<tr>
					<td>Thanks,</td>
			...
			";
			$headers = 'From: Mussino.com notification <notification@mussino.com>' . "\r\n" .
			'X-Mailer: PHP/' . phpversion();
	
			$headers  .= 'MIME-Version: 1.0' . "\r\n";
			$headers .= 'Content-type: text/html; charset=iso-8859-1' . "\r\n";
	
			$MAILSEND = @mail($soemail, "Artist have recorded in your session.", $body, $headers);
			
			$body = "
			<table width='100%' border='0' cellspacing='0' cellpadding='0' style='background:#F2F2F2; border:#CCCCCC;'>
				<tr>
					<td>Hello ".$_SESSION['SESS_FIRST_NAME']." ".$_SESSION['SESS_LAST_NAME']."</td>
				</tr>
				<tr><td height='6'></td></tr>
				<tr>
					<td>Your video is been recorded successfully, and below is the information about recording.</td>
				</tr>
				<tr><td height='6'></td></tr>
				<tr>
					<td><strong>Session:</strong> <a href='".SITE_WS_MPATH."/session.php?id=".$cat_id."'>".$sestitle."</a></td>
				...
				<tr>
					<td><strong>Title:</strong> ".$_POST['title']."</td>
				</tr>
				<tr><td height='6'></td></tr>
				<tr>
					<td>Thanks,</td>
			...
			";
			$MAILSEND = @mail($_SESSION['SESS_EMAIL'], "Your recording is now active.", $body, $headers);
		}		
	}
	echo '<script language="javascript">window.location="create-session.php?id='.$_POST['ses_id'].'";</script>';
}
    ...
    <div class="top-Pannel">
        ...
        <div class="vedioPrt">
            ...
            <div class="music-tweet">
                <h1>
                    <?=stripslashes($colles_Product['Title'])?>
                </h1>
            </div>
            <!-- / TWEET -->
            <!--
					<?php
					if(strtolower($pext)=='mp3' || strtolower($pext)=='wma')
					{
						?>
					...
					} elseif(strtolower($pext)=='avi' || strtolower($pext)=='3gp' || strtolower($pext)=='flv' || strtolower($pext)=='mkv' || strtolower($pext)=='mov' || strtolower($pext)=='mp4' || strtolower($pext)=='mpeg' || strtolower($pext)=='wmv' )
					{
					 ...
					 $uniqnm = str_replace(".","",str_replace("@","",stripslashes(Get_Single_Field("member_account_master","Email","Member_Account_Id",$_SESSION['SESS_ID']))));
					 
					 ?>
					 -->
            <div class="applet_container_record" id="recordnew" style="margin-bottom:8px;">
                <script type="text/javascript" src="<?=SITE_WS_PATH?>/script/AC_RunActiveContent.js"></script>
                <script language="javascript">
						if (AC_FL_RunContent == 0) {
							alert("This page requires AC_RunActiveContent.js.");
						} else {							
							var RecPlayer = AC_FL_RunContent(
								...
								'movie', 'record',
								'salign', '',
								'flashvars','server=<?php echo $rtmpserver;?>&uname=<?php echo $uniqnm;?>&cb=0&unm=<?php echo $uniqnm;?>&pg_url=http://192.168.1.16/mussino/'
								); //end AC code
            ...
            <script language="javascript">
						...
						function oit_rec_stopped(recorded_file_name)
						{
							calledfromrec = "yes";
							loadrecordings(recorded_file_name);							
						}
						
						
						function loadrecordings(recorded_file_name)
						{
							//e.preventDefault();
							var id = "#dialog";//$(this).attr('href');
							document.getElementById("dlgcntid").innerHTML = document.getElementById("upldfrmcntr_id").innerHTML;
							
							
							$("#dlgcntid #flvnm_id").val(recorded_file_name);
						...
						function recording_started(action)
						{
							if(action == 'start')
								$(".playable").data('playable').play();
							if(action == 'resume')
								$(".playable").data('playable').resume();
							if(action == 'pause')
								$(".playable").data('playable').pause();
							if(action == 'stop')
								$(".playable").data('playable').stop();
						}
					</script>
            <div style="float:left" class="fb-like" data-send="false" data-layout="button_count" data-width="150"  data-show-faces="true"></div>
              
            <div class="fblikes" style=" margin-top:0px; margin-left:3px; float:left"><a href="https://twitter.com/share" class="twitter-share-button" data-text="Enter <?=stripslashes($colles_Product['Title'])?> session today. Session ends on <?=get_date_format($colles_Product['Session_End_Date'])?> Signup Free! Mussino.com Virtual Music Industry " data-lang="en">Tweet</a> </div>
        </div>
        <!-- / vedioPrt -->
        <!--  session-info -->
        <div class="session-info">
            <div class="midPannel">
                ...
                <div style="padding: 10px 0 10px 10px; color:#000000; font-size:14px; font-weight:bold;">
                    <?=$_SESSION['sess_mess']?>
                    <?php //$_SESSION['sess_mess']='';?>
                </div>
                <?php } ?>
                <span class="heading timeLeftPro"><img width="27" src="http://mussino.com/images/timer.jpg" align="absmiddle"> <strong>Time Left :</strong></span> <span>
                <script language="JavaScript">
                ...
                <div class="product_info">
					...
					?>
                    <p> <span class="heading"><strong>Royalties : </strong><b>$
                        <?=$TOTAL_ROYALTIES;?>
                        </b> </span> <span class="heading"><strong>Posts : </strong><b>
                        <?=$totalPosts>0 ? $totalPosts : $colles_Product['Posts'];?>
                        </b></span> <span class="small_heading"><strong>Producer : </strong><a href="<?=SITE_WS_PATH?>/<?=trim($colles_Product['Member_Account_Id'])?>/<?=ucfirst(stripslashes(Get_Single_Field("member_account_master","First_Name","Member_Account_Id","$colles_Product[Member_Account_Id]")))?>" >
                        ...
                        </a></span> <span class="small_heading"><strong>Sound : </strong><a href="session.php?sound=<?=$colles_Product['Sound']?>">
                        <?=Get_Single_Field("sound_type_master","Sound_Type_Name","Sound_Type_Id","$colles_Product[Sound]")?>
                        </a></span> <span class="small_heading"><strong>Credits : </strong>
                        <?=$colles_Product['Product_Notes']?>
                        </span> <span class="small_heading"><strong>Genres : </strong><a title="<?=stripslashes(Get_Single_Field("category_master","Category_Name","Category_Id","$colles_Product[Category_Id]"))?>" href="session.php?id=<?=$colles_Product['Category_Id']?>">
                        <?=stripslashes(Get_Single_Field("category_master","Category_Name","Category_Id","$colles_Product[Category_Id]"))?>
                        </a></span>
                        <?php $shortDesc = str_replace("<p>", "", $colles_Product['Short_Desc']);  $shortDesc = str_replace("</p>", "", $shortDesc); ?>
                        <span class="small_heading"><strong>Sound Detail : </strong>
                        <?=stripslashes(substr($shortDesc,0,80)) ?>
                        </span> <span class="small_heading"><strong>Session Start : </strong>
                        <?=get_date_format($colles_Product['Session_Start_Date'])?>
                        </span> <span class="small_heading"><strong>Session End : </strong>
                        <?=get_date_format($colles_Product['Session_End_Date'])?>
                        </span> </p>
                </div>
                ...
                <div class="music-Btn2 session-upload-btn"> <a href="artist-membership-upgrade.php">Upload Audio</a> </div>
                <?php } else { ?>
                <div class="music-Btn2 session-upload-btn"> <a id="variousAudioUpload" href="#audio_upload_id">Upload Audio</a>
                            ...
                            <div style="padding: 5px 0 0 10px;">
                                <form id="frmUploadAudio" name="frmUploadAudio" method="post" action="" enctype="multipart/form-data" onSubmit="return Audio_Submitsite_Form(frmUploadAudio)">
                                    <ul>
                                        ...
                                        <li>
                                            <ul class="login">
                                                <li><strong>Audio File</strong> </li>
                                                <li>
                                                    <input type="file" name="Audio_File" id="Audio_File" class="input-text" size="30" onChange="validateFileExtensionAudio(this)">
                                                    <br />
                                                    <span style="font-family:Arial, Helvetica, sans-serif; font-size:11px; color:#25A6D5;" >[ Only upload mp3 ]</span></li>
                                            </ul>
                                        </li>
                                        <li> <br />
                                            <ul class="login">
                                                ...
                                                <li>
                                                    <textarea name="Archive_Content" cols="40" rows="6" ></textarea>
                                                </li>
                                            </ul>
                                        </li>
                                        <li> <br />
                                            <ul class="login">
                                                ...
                                                <li>
                                                    <input class="button" name="AudioButtonSubmit" type="submit" value="Submit" >
                                                </li>
                                            </ul>
                                        </li>
                                    </ul>
                ...
                <?php if($typ=='Artist') { ?>
                <?php if(mysql_num_rows($resultMemberShip)==0) { ?>
                <div class="music-Btn2 session-upload-btn"> <a href="artist-membership-upgrade.php">Upload Video</a> </div>
                <?php } else { ?>
                <div class="music-Btn2 session-upload-btn"> <a id="variousVideoUpload" href="#video_upload_id">Upload Video</a>
                    <div style="display: none;">
                                    ...
                                    <ul>
                                        ...
                                        <li>
                                            <ul class="login">
                                                ...
                                                <li>
                                                    <input type="file" name="Video_File" id="Video_File" class="input-text" size="30" onChange="validateFileExtensionVideo(this)">
                                                    <br />
                                                    <span style="font-family:Arial, Helvetica, sans-serif; font-size:11px; color:#25A6D5;" >[ Only upload flv ]</span></li>
                                            </ul>
                                        </li>
                                        <li> <br />
                                            <ul class="login">
                                        ...
                                        <li>
                                            <ul class="login">
                                                ...
                                                <li>
                                                    <input class="button" name="VideoButtonSubmit" type="submit" value="Submit" >
                                                </li>
                                            </ul>
                                        </li>
                                    </ul>
        ...
        <div class="cl"></div>
        <br />
		<div class="track playable" data-url="products/small_video/<?php echo $pfileName;?>"><?php echo stripslashes($colles_Product['Title'])?></div>
		<div class="unsign_artist_video" id="recordings_vids_cntr_id">
			
		</div>
        <!--<div class=""> <strong>Studio Booth:</strong> Welcome to the recording session area. Above you will see a media player with the track and details below. All lyrics posted in the notebook on the right, will be copywriting and dated under you profile account. </div>-->
    ...
    <div class="top-Pannel" style="display:none">
        <div class="rgtPannel">
            ...
            <form name="chFrm" method="post" action="">
                <div style="width:669px;"> <font style="color:#333; font-size:12px; float:left;"><strong>Text session 8 - 16 bars only</strong></font>
                    <h3 style="display:inline; width:190px; float:right; margin-bottom:0px;">Character limit:<span>
                        <input readonly type="text" name="remLen1" size="3" maxlength="3" value="1600">
                        max</span> </h3>
...
onKeyUp="textCounter(document.chFrm.Lyrics,document.chFrm.remLen1,1200)"><?=stripslashes($collesTextLyrics['Lyrics'])?>
</textarea>
                </p>
                <div class="alignR">
                    <?php if($_SESSION['SESS_ACCOUNT_TYPE']=='Contest Judge')  { ?>
                    <?php /*?><div class="gray-btn-1">
                  <span>
                    <span>
                ...
                <input type="button" name="history" value="Notebook" onmouseover="showStuff('msg_confirm_id');" onmouseout="hideStuff('msg_confirm_id');" onclick="window.location.href='history.php?id=<?=$_REQUEST[id]?>'" />
                    ...
                    <div class="popupConfirm scroll" style="display:none;" id="msg_confirm_id">
                        <div> <span  align="left">
                            <!--<h2>Info</h2>-->
                            <p>
                                <?=stripslashes(Get_Single_Field("notebook_msg_master","Notebook_Msg_Details","Notebook_Msg_Id ","1"))?>
                            </p>
                            </span> </div>
                    </div>
                    <?php /*?><div class="gray-btn-1">
        ...
        <div class="session-mid-advertisement" style="float:right; width:267px">
            <h2 class="profile_section_header" align="right">Session Menu</h2>
            <br />
            <a  href="https://twitter.com/share" class="twitter-share-button size_small" data-text="Enter <?=stripslashes($colles_Product['Title'])?> session today. Session ends on <?=get_date_format($colles_Product['Session_End_Date'])?> Signup Free! Mussino.com Virtual Music Industry " data-lang="en" style="text-align:center">Tweet to all your followers</a> <br />
            <div class="music-Btn"> <a class="size_small" href="http://www.mussino.com/product.php">Create New Session</a> </div>
            <div class="music-Btn"> <a class="size_small" href="http://www.mussino.com/music-store.php">Sell Beats</a> </div>
            <div class="music-Btn"> <a href="/send_to_friend/popup.php" title="Send to Friend" class="sendtofriend size_small">Send Page To Friend</a> </div>
            <div class="music-Btn"> <a class="size_small" href="<?=SITE_WS_PATH?>/message.php?uid=<?=md5($_SESSION['SESS_ID'])?>">Invite People</a> </div>
            <div class="session-mid-rightside" style="display:none;">
                <?php } else { ?>
                <div class="session-mid-rightside">
                    <?php }?>
                    <div class="music-session-btn">
                        <h2 class="profile_section_header" align="right">Session Menu</h2>
                        <div class="music-Btn">
                            <?php if(mysql_num_rows($resultMemberShip)==0) { ?>
                            <a class="size_small" href="artist-membership-upgrade.php" target="_blank">Membership download beat</a>
                            <?php } else { ?>
                            <script language="javascript">
				...
				function handleDownloadResponse(){
					 ...
					 if(httpobj.readyState==4)
					 {
						 ...
						 document.getElementById('download_frm_id').innerHTML = text;
                            ...
                            <div class=""> <a class="size_small" id="variousDownload" href="#download_id">Get session beat</a>
                                <div style="display: none;">
                                            ...
                                            <form id="frmDownload" name="frmDownload" method="post" action="">
                                                <ul>
                                                    ...
                                                    <li>
                                                        <ul>
                                                            <li>
                                                                <input type="checkbox" name="rights" value="1" class="input-text" onkeypress="if(event.keyCode==13) {return downloadRequest();}" />
                                                                I accept to legal rights</li>
                                                        </ul>
                                                    </li>
                                                    <li>
                                                        <ul class="login">
                                                            <li>&nbsp;</li>
                                                            <li>
                                                                <input class="button" name="buttonSubmit" type="button" value="Download" onclick="return downloadRequest();" onkeypress="if(event.keyCode==13) {return downloadRequest();}">
                                                            </li>
                                                        </ul>
                                                    </li>
                                                </ul>
                            ...
                            <?php } ?>
                            <!-- <p  class="size_small">Get session beat</p> -->
                        </div>
                        <br />
                        <div class="music-Btn scroll" type="button" name="post" id="post_button" value="Post" onclick="showStuff('confirm_id');" disabled="disabled" style=" color:#999;">
                            <p  class="size_small">Post Lyrics</p>
                        </div>
                        <br />
                        <?php if($_SESSION['SESS_ACCOUNT_TYPE']=='Contest Judge')  { ?>
                        <div class="music-Btn"   name="history" onmouseover="showStuff('msg_confirm_id');" onmouseout="hideStuff('msg_confirm_id');" onclick="window.location.href='contest-juge-history.php?id=<?=$_REQUEST[id]?>'" >
                            <?php } else if($_SESSION['SESS_ACCOUNT_TYPE']=='Artist') { ?>
                            <div class="music-Btn"   name="history" onmouseover="showStuff('msg_confirm_id');" onmouseout="hideStuff('msg_confirm_id');" onclick="window.location.href='history.php?id=<?=$_REQUEST[id]?>'" >
                                <?php } ?>
                                <p class="size_small">Notebook</p>
                            </div>
                            <br />
                            <div class="music-Btn scroll1" type="button" value="Report" id="report_id" onclick="showStuff('report');hideStuff('report_id');" >
                                <p class="size_small">Report Issue</p>
                            </div>
                            <div class="dicBox">
                                <form class="fop-searchbox" id="formname" name="formname" method="get" style="padding: 10px; width: 260px; font-family: Arial,Helvetica,sans-serif; text-align: center; -moz-border-radius: 15px;
border-radius: 15px;">
                                    <img class="fop-logo" src="http://www.macmillandictionary.com/license/default/external/images/macmillandictionary165.gif" alt="Macmillan Online Dictionary" title="Macmillan Online Dictionary" style="margin-bottom: 4px;" width="90"/> <br />
...
border-radius: 15px;"/>
                                    <br />
                                    <br />
                                    <input class="" value="Search" type="image" src="images/find.png" alt="Click here to start the search in Macmillan Online Dictionary" title="Click here to start the search in Macmillan Online Dictionary" style="cursor: pointer; vertical-align: top; width:30px; height:30px;"/>
                                </form>
                            </div>
                        </div>
                    </div>
                    ...
                    <div> <span  align="center">
                        ...
                        <form id="frmReport" name="frmReport"  method="post" action="">
                            <table width="90%" cellspacing="2" cellpadding="2">
                                <?php if($_SESSION["sess_mess"]!='') { ?>
                                <div style="color:#757575; padding: 0 0 0 26%;">
                                    <?=$_SESSION["sess_mess"]?>
                                    <? $_SESSION["sess_mess"]='';?>
                                </div>
                                <?php } ?>
                                ...
                                <tr>
                                    <td width="40%"><input type="hidden" name="id" value="<?=$_REQUEST['id']?>" />
                                    </td>
                ...
                <div class="popupConfirm scroll" style="display:none;" id="confirm_id">
                    <div> <span  align="center" id="view_post_lyrics_result">
                        <h2>Wait Confirmation</h2>
                        <p>Are you sure you are ready to post your lyrics? You can not edit anymore durning this session.<br />
                            Choose below</p>
                        <a href="javascript:void(0);" onclick="return post_lyrics('<?=$typ?>','<?=$_REQUEST['id']?>','<?=$_SESSION['SESS_ID']?>');"><strong>Yes!</strong></a> <a href="javascript:void(0);" onclick="hideStuff('confirm_id');" ><span>No! I'm not Ready</span></a> </span> </div>
                </div>
                <div class="cl"></div>
            ...
            <div class="top-Pannel" style="display:none;">
                <div class="rgtPannel history info" style="width:500px; float:left">
                    <h2 class="profile_section_header"><img src="http://mussino.com/images/icon/notebook_icon.jpg" align="absmiddle" /> Songwriters Latest Posts</h2>
                    <div class="scroller">
                        <?php
			$ct=1;
            ...
            ?>
                        <div class="postBlock" style="float:left; width:100%; padding-top:10px;">
                            ...
                            <div style="padding-left:65px; padding-right:10px">
                                <p class="sesuname-title">
                                    <?=ucfirst(stripslashes(Get_Single_Field("member_account_master","First_Name","Member_Account_Id","$colles_history_post[Member_Account_Id]")))?>
                                </p>
                                <?=stripslashes(substr($colles_history_post['Lyrics'],0,120));?>
                                <?php if(strlen($colles_history_post['Lyrics'])>60) { echo '...'; } ?>
                                <!-- TWEET -->
                                <div class="songwriter-post-tweet-pos" >
                                    <div class="fb-like" data-send="false" data-href="http://www.mussino.com?lyricspost=<?php echo ($ct) ?>" data-layout="button_count" data-show-faces="true"></div>
                                    <a href="https://twitter.com/share" class="twitter-share-button"  data-url="http://www.mussino.com?musicpost=<?php echo ($ct) ?>"  data-text="Vote or Challenge <?=ucfirst(stripslashes(Get_Single_Field("member_account_master","First_Name","Member_Account_Id","$colles_history_post[Member_Account_Id]")))?> on Mussino.com:  <?=stripslashes(substr($colles_history_post['Lyrics'],0,60));?> <?php if(strlen($colles_history_post['Lyrics'])>90) { echo '...'; } ?>" data-lang="en">Tweet</a> </div>
                                <!-- / TWEET -->
                            </div>
                            <div class="cl"></div>
                            <div style=" padding:0px 0px 0 55px; font-size:13px; color:#CCC; float:right">
                                <?=$diff;?>
                            </div>
                            <div class="cl"></div>
                        </div>
			...
			$ct++;
                        ...
                        <div class="nopost">
                            <?php if($_SESSION['SESS_ACCOUNT_TYPE']=='Musician')  { ?>
                            <h2>No songwriters yet.<BR />
                                Use tools below to help get it started.</h2>
                            <br />
                            <script>
...
$('.popup').click(function(event) {
    ...
    return false;
                            ...
                            <a href="/send_to_friend/popup.php" target="_blank" title="Send to Friend" class="sendtofriend"> <img src="../images/invite-songwriter.png" /></a> <a href="https://twitter.com/share" class="twitter popup" data-text="Check out my session on Mussino.com" data-lang="en"><img src="../images/promote-session.png" /></a>
                            <?php } else {  ?>
                            <h2>Be the first to post to this session.<BR />
                                Use tools below to invite.</h2>
                            <br />
                            <script>
				$(document).ready(function() {
$('.popup').click(function(event) {
    ...
    return false;
                            ...
                            <a href="/send_to_friend/popup.php" target="_blank" title="Send to Friend" class="sendtofriend"> <img src="../images/invite-songwriter.png" /></a> <a href="https://twitter.com/share" class="twitter popup" data-text="Check out my session on Mussino.com" data-lang="en"><img src="../images/promote-session.png" /></a> </div>
                        <?php }  ?>
                        <?php }
			?>
                        <?php
			...
			?>
                        <div class="postBlock" style="float:left; width:100%; padding-top:10px;">
                            ...
                            <div style="padding-left:65px; padding-right:10px">
                                <p class="sesuname-title">
                                    <?=ucfirst(stripslashes(Get_Single_Field("member_account_master","First_Name","Member_Account_Id","$collesAudio[Member_Account_Id]")))?>
                                </p>
                                Click to play
                                <?=ucfirst(stripslashes(Get_Single_Field("member_account_master","First_Name","Member_Account_Id","$collesAudio[Member_Account_Id]")))?>
                                audio session
                                <p><a href="artist-preview-player.php?id=<?=$collesAudio['Lyrics_Post_Audio_Id']?>" title="<?='Artist '.ucfirst($m_name).' '.ucfirst(strtolower($collesAudio['Lyrics_Audio_Type'])) .' File'?>" rel="gb_page_center[640, 360]" class="global-box"><img src="images/play-btn.png" alt="" width="33" height="33" align="absmiddle" /> Play Audio</a></p>
                                <div style="clear:both;">
                                    <!-- TWEET -->
                                    <div class="songwriter-post-tweet" >
                                        <div class="fb-like" data-send="false" data-href="http://www.mussino.com?audiopost=<?php echo ($ct) ?>" data-layout="button_count" data-show-faces="true"></div>
                                        <a href="https://twitter.com/share" class="twitter-share-button"  data-url="http://www.mussino.com?audiopost=<?php echo ($ct) ?>"  data-text="Check out <?=ucfirst(stripslashes(Get_Single_Field("member_account_master","First_Name","Member_Account_Id","$collesAudio[Member_Account_Id]")))?> on Mussino.com <?=stripslashes(substr($colles_history_post['Lyrics'],0,120));?> <?php if(strlen($colles_history_post['Lyrics'])>60) { echo '...'; } ?>" data-lang="en">Tweet</a> </div>
                                    <!-- TWEET -->
                                </div>
                                <div class="cl"></div>
                                <div style=" padding:0px 0px 0 55px; font-size:13px; color:#CCC; float:right">
			...
			?>
                            <div class="postBlock" style="float:left; width:100%; padding-top:10px;">
                                ...
                                <div style="padding-left:65px; padding-right:10px">
                                    <p class="sesuname-title">
                                        <?=ucfirst(stripslashes(Get_Single_Field("member_account_master","First_Name","Member_Account_Id","$collesVideo[Member_Account_Id]")))?>
                                    </p>
                                    Click to play
                                    <?=ucfirst(stripslashes(Get_Single_Field("member_account_master","First_Name","Member_Account_Id","$collesVideo[Member_Account_Id]")))?>
                                    video session <a href="artist-preview-player.php?id=<?=$collesVideo['Lyrics_Post_Audio_Id']?>" title="<?='Artist '.ucfirst($m_name).' '.ucfirst(strtolower($collesVideo['Lyrics_Audio_Type'])) .' File'?>" rel="gb_page_center[640, 360]" class="global-box"><img src="images/play-btn.png" alt="" width="33" height="33" align="absmiddle" /> Play Video</a>
                                    <div style="clear:both;">
                                        <!-- TWEET -->
                                        <div class="songwriter-post-tweet-pos" >
                                            <div class="fb-like" data-send="false" data-href="http://www.mussino.com?videopost=<?php echo ($ct) ?>" data-layout="button_count" data-show-faces="true"></div>
                                            <a href="https://twitter.com/share" class="twitter-share-button" data-url="http://www.mussino.com?videopost=<?php echo ($ct) ?>" data-text="Check out <?=ucfirst(stripslashes(Get_Single_Field("member_account_master","First_Name","Member_Account_Id","$collesVideo[Member_Account_Id]")))?> on Mussino.com <?=stripslashes(substr($colles_history_post['Lyrics'],0,120));?> <?php if(strlen($colles_history_post['Lyrics'])>60) { echo '...'; } ?>" data-lang="en">Tweet</a> </div>
                                        <!-- TWEET -->
                                    </div>
                                    <div class="cl"></div>
                                    <div style=" padding:0px 0px 0 55px; font-size:13px; color:#CCC; float:right;">
                    ...
                    <!--<div style=" width:450px; float:right; margin-top:20px;">
                        <h2 class="profile_section_header"><img src="http://mussino.com/images/tsupport.png" align="absmiddle" width="20" /> Mussino tweet support and latest updates. </h2>
                        <?php /*?>      <a class="twitter-timeline"  href="https://twitter.com/MussinoNetwork" data-widget-id="273614994208342017">Tweets by @MussinoNetwork</a>

<script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>
<?php */?>
                        <a class="twitter-timeline" href="https://twitter.com/MussinoSupport" data-widget-id="273635674731716609">Tweets by @MussinoSupport</a>
                        <script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>
                    </div>
                    <div class="cl"></div>-->
                </div>
...
.page_title_cntr{width:100%; float:left;}
.games_settings_button_cntr{width:26%; padding-right:8px; float:right; text-align:right}
</style>
<div id="dialog" class="window">
    <div class="dialog_container" id="dialog_container_id">
        <div id="dlgcntid" style="color:#01549F; font-weight:bold; font-size:14px;"> Loading.... </div>
    </div>
</div>
<div id="mask"></div>
<div style="display:none;" id="upldfrmcntr_id">
    <div class="form-container full">
        <form action="" method="post" name="frmUploadRecording" id="frmUploadRecording_id">
            <ul class="fl">
                <li></li>
                <li style="color:#000000;">Enter your recorded session detail below to upload it.<img src="greybox/w_close.gif" style="float:right; cursor:pointer;" onclick="closebox();" /></li>
                <li>
                    <div class="caption-3">Title:</div>
                    <div class="input-2">
                        <input type="text" class="input-text textinput" name="title" style="width:500px;">
                    </div>
                    <div class="cl"></div>
                </li>
                <li>
                    <div class="caption-3">Description:</div>
                    <div class="input-2">
                        <textarea class="input-text textinput" name="descr" id="descr_id" style="width:485px; height:100px;"></textarea>
                    </div>
                    <div class="cl"></div>
                </li>
                <li class="last">
                    <div style="padding-left:0px;" class="input-2">
                        <input type="submit" value="Upload my Recording!" name="buttonSubmit" class="button">
                    </div>
                    <div class="cl"></div>
                </li>
            </ul>
            <input type="hidden" name="flvnm" id="flvnm_id" />
			<input type="hidden" name="ses_id" value="<?php echo $_GET['id'];?>" />
        </form>
        <div class="cl"></div>
    </div>
</div>
